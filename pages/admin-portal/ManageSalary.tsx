...
const SalaryManagementModal: React.FC<{
    ...
    const handleRemovePayment = (paymentId: string) => {
        if(window.confirm("Are you sure you want to permanently delete this payment record? This action is highly sensitive and cannot be undone.")) {
            setFacultyData(prev => ({
                ...prev,
                salaryHistory: prev.salaryHistory?.filter(p => p.id !== paymentId) || [],
            }));
        }
    }
                    ...
                    <div>
                        ...
                        <div className="max-h-60 overflow-y-auto border rounded-md">
                            <table className="w-full text-sm">
                                <thead className="bg-gray-100 sticky top-0"><tr><th className="p-2">Date</th><th className="p-2">Amount</th><th className="p-2">Category</th><th className="p-2">Method</th><th className="p-2">Notes</th><th className="p-2"></th></tr></thead>
                                <tbody>
                                    {facultyData.salaryHistory?.sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(p => (
                                        <tr key={p.id} className="border-b"><td className="p-2">{p.date}</td><td className="p-2">NPR {p.amount.toLocaleString()}</td><td className="p-2">{p.category}{p.otherCategory ? ` (${p.otherCategory})`: ''}</td><td className="p-2">{p.method}</td><td className="p-2">{p.notes}</td>
                                        <td className="p-2 text-center">
                                            <button type="button" onClick={() => handleRemovePayment(p.id)} className="text-red-500 hover:text-red-700 font-bold" title="Delete this payment record">&times;</button>
                                        </td>
                                        </tr>
                                    ))}
                                </tbody>
