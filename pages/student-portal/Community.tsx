...
import { CHAT_MESSAGES, STUDENTS, TEACHER_QUESTIONS, SHARED_RESOURCES, COURSES } from '../../constants.ts';
import type { Student, ChatMessage, TeacherQuestion, ChatAttachment, SharedResource } from '../../types.ts';
...
const Community: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'chat' | 'ask' | 'resources'>('chat');
        ...
        <div>
            ...
            <div className="mb-6 flex border-b">
                <button 
                    ...
                    className={`py-2 px-6 font-semibold ${activeTab === 'chat' ? 'border-b-2 border-brand-red text-brand-red' : 'text-gray-500'}`}
                >
                    Study Groups
                </button>
                <button 
                    onClick={() => setActiveTab('ask')} 
                    className={`py-2 px-6 font-semibold ${activeTab === 'ask' ? 'border-b-2 border-brand-red text-brand-red' : 'text-gray-500'}`}
                >
                    Ask a Teacher
                </button>
                 <button 
                    onClick={() => setActiveTab('resources')} 
                    className={`py-2 px-6 font-semibold ${activeTab === 'resources' ? 'border-b-2 border-brand-red text-brand-red' : 'text-gray-500'}`}
                >
                    Shared Resources
                </button>
            </div>
