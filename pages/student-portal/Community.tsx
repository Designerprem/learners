...
import { COURSES, STUDENTS, CHAT_MESSAGES, TEACHER_QUESTIONS, FACULTY_MEMBERS } from '../../constants';
import type { Student, ChatMessage, TeacherQuestion, ChatAttachment } from '../../types';
...
const AskTeacher = () => {
    ...
    // Filter for questions asked by the current student (Alex Doe, ID 1)
    const myQuestions = questions.filter(q => q.studentId === 1);
...
const StudentChat = () => {
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        const newMsg: ChatMessage = {
            ...
            studentId: 1, // Current user is Alex Doe
            ...
            <div className="flex-1 bg-white p-6 rounded-lg shadow-md flex flex-col h-[70vh]">
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                    {messages.map(msg => {
                        ...
                        const isCurrentUser = student.id === 1; // Assuming current user is Alex Doe
                        return (
                            <div key={msg.id} className={`flex items-start gap-3 ${isCurrentUser ? 'justify-end' : ''}`}>
                                {!isCurrentUser && <img src={student.avatarUrl} alt={student.name} className="w-10 h-10 rounded-full" />}
                                <div>
                                    <div className={`text-sm p-3 rounded-lg max-w-sm ${isCurrentUser ? 'bg-brand-red text-white' : 'bg-gray-100'}`}>
                                        <p className={`font-bold mb-1 ${isCurrentUser ? 'hidden' : 'block'}`}>{student.name}</p>
                                        {msg.text && <p>{msg.text}</p>}
                                        {msg.attachment && <RenderAttachment attachment={msg.attachment} />}
                                    </div>
                                    <p className={`text-xs text-gray-400 mt-1 ${isCurrentUser ? 'text-right' : ''}`}>{msg.timestamp}</p>
                                </div>
                                {isCurrentUser && <img src={student.avatarUrl} alt={student.name} className="w-10 h-10 rounded-full" />}
                            </div>
                        );
